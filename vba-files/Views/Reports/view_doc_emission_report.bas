Attribute VB_Name = "view_doc_emission_report"
...
Function publish(project_selected_id As String, date_selected As String)
    ...
    excel_report_full_file_path = config_sheet.Range("CONF_DEFAULT_FORM_PATH").Value & "\RELATORIO_DOCUMENTOS_EMITIDOS.xlsb"
...
Private Function populate_report_file(project_id As String, date_selected As String)
    ...
    Do Until respQuery.EOF
        ...
        iObject.ListColumns("REV.").DataBodyRange(i).Value = XdbFactory.getData(respQuery, "rev_code")
        iObject.ListColumns("TE").DataBodyRange(i).Value = XdbFactory.getData(respQuery, "issue")
        iObject.ListColumns("REVISÃO").DataBodyRange(i).Value = XdbFactory.getData(respQuery, "next_review")
        iObject.ListColumns("EMISSÃO").DataBodyRange(i).Value = XdbFactory.getData(respQuery, "next_issue")
        iObject.ListColumns("STATUS").DataBodyRange(i).Value = XdbFactory.getData(respQuery, "status")
